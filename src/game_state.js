 ...
 *   objectiveStatus: 'active'|'complete'|'failed',
 *   missionStatus: 'briefing'|'in_progress'|'success'|'failure',
...
export class GameState {
    ...
    /** @type {number} */
    missionIndex = 0;
    /** @type {'briefing'|'in_progress'|'success'|'failure'} */
    missionStatus = 'briefing';
    /** @type {ObjectiveState} */
    objective = {
        id: 'vs0_secure_intel',
        title: 'Secure intel from the mansion office and survive the extraction window.',
        status: 'active',
    };
    ...
    /** @param {{runSeed?: number, missionIndex?: number, missionSeed?: number}=} options */
    setupLevel(options = {}) {
        if (typeof options.runSeed === 'number') {
            this.runSeed = options.runSeed;
        }
        if (typeof options.missionIndex === 'number') {
            this.missionIndex = options.missionIndex;
        }
        this.missionSeed = typeof options.missionSeed === 'number'
            ? options.missionSeed
            : this.deriveMissionSeed(this.missionIndex);
        this.missionMap.rng.seed(this.missionSeed);
        ...
        this.message = `Mission started (run seed ${this.runSeed}, mission seed ${this.missionSeed})`;
    }

    /** @param {number} missionIndex */
    deriveMissionSeed(missionIndex) {
        return this.runSeed + missionIndex * 9973;
    }
    ...
    getView() {
        return {
            ...
            objectiveStatus: this.objective.status,
